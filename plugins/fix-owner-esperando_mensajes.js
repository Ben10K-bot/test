...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  const sessionPath = './MyBen10 Session/';
  try {
    if (!existsSync(sessionPath)) {
      return await conn.sendMessage(m.chat, {text: '*[❗] مجلد MyBen 10Session غير موجود أو فارغ.*'}, {quoted: m});
    }
    const files = await s.readdir(sessionPath);
    let filesDeleted = 0;
